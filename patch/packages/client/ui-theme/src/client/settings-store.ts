...
type AppearanceRowActions = {
  sync: (draft: AppearanceRowState, preference: ThemePreference, theme: string, themes: readonly ThemeDefinition[], revision: number) => void
}
...
export function createAppearanceRowStore(): EngineStoreHandle<AppearanceRowState, AppearanceRowActions> {
  return defineStore({
    init: (): AppearanceRowState => ({ preference: 'system', theme: 'default', themes: [], revision: -1 }),
    actions: {
      sync: (d, preference: ThemePreference, theme: string, themes: readonly ThemeDefinition[], revision: number) => {
