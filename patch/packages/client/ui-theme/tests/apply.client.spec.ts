...
async function bench(isLoopback = true) {
  ...
  const mutate = vi.fn((request: { ops: { path?: string[]; value: string }[] }) => {
    ...
    if (op.path?.includes('theme')) theme = op.value
  ...
  return {
    ...
    setHostTheme: (next: string) => { theme = next },
  }
}
...
describe('ui-theme apply', () => {
  ...
  it('routes style-theme writes through setThemeId into the theme field', async () => {
    const b = await bench()
    declareItems(b.slots)
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    const theme = b.ctx.get('theme') as ThemeRuntime
    const { instance, face } = faceOf(b.slots)
    face.setThemeId('aurora')
    expect(theme.getTheme().theme).toBe('aurora')
    expect(theme.getTheme().active.id).toBe('aurora')
    expect(instance.getSnapshot().theme).toBe('aurora')
    await vi.waitFor(() => {
      expect(b.mutate).toHaveBeenCalledWith(expect.objectContaining({
        ops: [expect.objectContaining({ path: ['theme'], value: 'aurora' })],
      }))
    })
  })
