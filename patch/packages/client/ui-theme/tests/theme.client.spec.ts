...
describe('ThemeRuntime', () => {
  it('defaults to the system preference resolved against prefers-color-scheme', () => {
    ...
    // Built-ins: the light/dark pair plus the shipped style themes.
    expect(snapshot.themes.map(t => t.id)).toEqual([
      'light', 'dark', 'academic', 'aurora', 'cyberpunk', 'facebook', 'liquidglass',
      'lychee', 'magazine', 'matrix', 'minimal', 'mint', 'morandi', 'nature',
      'palace', 'sunset', 'tech', 'typewriter', 'vaporwave', 'vibrant', 'violet',
    ])
  })
  ...
  it('setThemeId switches the style theme, writes through the scope, and republishes', () => {
    const { theme, events, host } = make()
    theme.setThemeId('aurora')
    expect(theme.getTheme().theme).toBe('aurora')
    expect(theme.getTheme().active.id).toBe('aurora')
    expect(theme.getTheme().active.colorScheme).toBe('light')
    expect(host.set).toHaveBeenCalledWith('theme', 'aurora')
    expect(events).toHaveLength(1)
  ...
  it('a style theme rides either palette: its colorScheme follows the preference', () => {
    const { theme } = make()
    theme.setThemeId('aurora')
    theme.setTheme('dark')
    expect(theme.getTheme().theme).toBe('aurora')
    expect(theme.getTheme().active.id).toBe('aurora')
    expect(theme.getTheme().active.colorScheme).toBe('dark')
    // Style themes keep their metadata on the active definition.
    expect(theme.getTheme().active.name).toBe('极光')
  })
  ...
  it('throws on unknown setTheme ids, unknown style themes, duplicate registration, and the system id', () => {
    const { theme } = make()
    expect(() => { theme.setTheme('sepia' as never) }).toThrow('not a built-in mode')
    expect(() => { theme.setThemeId('sepia') }).toThrow('not registered')
  ...
  it('registered themes join the snapshot; disposing the active one resets to default', () => {
    ...
    expect(theme.getTheme().themes.map(t => t.id)).toEqual([
      'light', 'dark', 'academic', 'aurora', 'cyberpunk', 'facebook', 'liquidglass',
      'lychee', 'magazine', 'matrix', 'minimal', 'mint', 'morandi', 'nature',
      'palace', 'sunset', 'tech', 'typewriter', 'vaporwave', 'vibrant', 'violet', 'sepia',
    ])
    theme.setThemeId('sepia')
    expect(theme.getTheme().active.tokens['--dsw-alias-bg-base']).toBe('red')
    dispose()
    expect(theme.getTheme().theme).toBe('default')
    ...
    // register + setThemeId + dispose = three publishes; disposer is idempotent.
    expect(events.length).toBe(3)
    dispose()
    expect(events.length).toBe(3)
  })

  it('disposing an inactive theme keeps the active preference and style theme', () => {
    const { theme } = make()
