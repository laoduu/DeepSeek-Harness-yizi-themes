...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    description: 'Theme registry and preference owner. `light`/`dark` are built in (the base stylesheets carry both palettes); style themes register alias-layer overrides and/or pair with a `body.theme-<id>` overrides sheet shipped by this package. Reads go through getTheme; preference writes only through setTheme (built-in modes) and style-theme writes through setThemeId; continuous sync only through the `theme/change` event. overrideTokens stacks partial token layers over the active theme without touching the registry. The service holds the `prefers-color-scheme` media query (environment sensing, not presentation) and re-emits when the OS scheme flips while the preference is `system`.',
    methods: [
      {
      ...
      },
      {
        ...
        description: 'Switch the built-in preference (light/dark/system) — the only preference write entry; every accepted value emits `theme/change` and persists. Style-theme selection goes through setThemeId.',
        parameters: [{ name: 'id', description: 'a built-in preference id; unknown ids throw.' }],
      },
      {
        signature: 'setThemeId(id: string): void',
        description: 'Switch the style theme — the orthogonal appearance dimension. `default` clears the selection; any other id must be a registered theme.',
        parameters: [{ name: 'id', description: 'a registered theme id or {@link DEFAULT_THEME}.' }],
      },
      {
        ...
        returns: 'disposer. Disposing the theme backing the active style selection resets it to the default so the UI never keeps tokens of an unregistered theme.',
      },
      {
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  },
  {
    ...
    summary: 'Theme state changed (preference switched, style theme switched, registry updated, or the OS color scheme changed while the preference is `system`).',
    description: 'Theme state changed (preference switched, style theme switched, registry updated, or the OS color scheme changed while the preference is `system`).',
